...
def main(argv):
    ...
    for file_name in FILENAMESOUT:
        ...
        if isthere :
            recreate = input("%s already exists. Do you want to recreate the file [y/n]?"%fileout)
            if recreate == "y":
    ...
    for particle, file_name, fileout in zip(run_conf["Ntuple types"],FILENAMESIN,FILENAMESOUT):
        ...
        if particle == "muons" or particle == "electrons" or particle == "pions":
            ...
            if "deg" in file_name :
                ...
                command_analysis = "python3 analysis.py --filein %s --fileout %s --listVariables %s --listSelections %s %s"%(filein,fileout,LISTVAR_FIXED_THETA,CUTS_FIXED_THETA,ANALYSIS_FLAGS)
                if bool(test) :
            ...
            if "GeV" in file_name :
                ...
                command_analysis = "python3 analysis.py --filein %s --fileout %s --listVariables %s --listSelections %s %s"%(filein,fileout,LISTVAR_FIXED_PT,CUTS_FIXED_PT,ANALYSIS_FLAGS)
                if bool(test) :
                    command_analysis += " --verbose "
                    print(command_analysis)
                os.system(command_analysis)
        ...
        #complex events
            ...
            command_analysis = "python3 analysis.py --filein %s --fileout %s --listVariables %s --listSelections %s %s"%(filein,fileout,LISTVAR,CUTS,ANALYSIS_FLAGS)
            if bool(test) :
                command_analysis += " --verbose "
                print(command_analysis)
            os.system(command_analysis)
