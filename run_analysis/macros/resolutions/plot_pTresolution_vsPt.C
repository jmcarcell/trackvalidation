...
void plot_pTresolution_vsPt(char *inputFileName){
  ...
  while ((key = (TKey*)next())){
	...
	fitPar->GetEntry(k);
	resolutions_10deg[iter10] = sigma;
	resolutionsErr_10deg[iter10] = sigmaErr;
	iter10++;
      }
    }
      ...
      for(int k=0; k<entries; k++){
	fitPar->GetEntry(k);
	resolutions_30deg[iter30] = sigma;
	resolutionsErr_30deg[iter30] = sigmaErr;
	iter30++;
      }
    }
      ...
      for(int k=0; k<entries; k++){
	fitPar->GetEntry(k);
	resolutions_50deg[iter50] = sigma;
	resolutionsErr_50deg[iter50] = sigmaErr;
	iter50++;
      }
    }
      ...
      for(int k=0; k<entries; k++){
	fitPar->GetEntry(k);
	resolutions_70deg[iter70] = sigma;
	resolutionsErr_70deg[iter70] = sigmaErr;
	iter70++;
      }
    }
      ...
      for(int k=0; k<entries; k++){
	fitPar->GetEntry(k);
	resolutions_89deg[iter89] = sigma;
	resolutionsErr_89deg[iter89] = sigmaErr;
	iter89++;
      }
    }
  }
  ...
  */
  ...
  gr_10deg->GetYaxis()->SetTitle("#sigma(#Deltap_{T}/p^{2}_{T,true}) [GeV^{-1}]");
  gr_10deg->GetYaxis()->SetTitleOffset(1.3);
  gr_10deg->GetXaxis()->SetRangeUser(0.9,1600);
  gr_10deg->Draw("ap");
  gr_10deg->SetMaximum(1e-1);
  gr_10deg->SetMinimum(1e-5);
  ...
  if( inputFile.find(muon) != -1 )
    c->SaveAs("figures/muons/MomRes_vs_pt.eps");
  else if( inputFile.find(electron) != -1 )
    c->SaveAs("figures/electrons/MomRes_vs_pt.eps");
  else if( inputFile.find(pion) != -1 )
    c->SaveAs("figures/pions/MomRes_vs_pt.eps");
