...
void plot_z0resolution_vsPt(char *inputFileName){
  ...
  */
  ...
  if( inputFile.find(muon) != -1 )
    c->SaveAs("figures/muons/z0Res_vs_pt.eps");
  else if( inputFile.find(electron) != -1 )
    c->SaveAs("figures/electrons/z0Res_vs_pt.eps");
  else if( inputFile.find(pion) != -1 )
    c->SaveAs("figures/pions/z0Res_vs_pt.eps");
