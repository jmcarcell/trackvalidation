...
void plot_phiresolution_vsPt(char *inputFileName){
  ...
  while ((key = (TKey*)next())){
	...
	fitPar->GetEntry(k);
	resolutions_10deg[iter10] = sigma * 17.4533;
	resolutionsErr_10deg[iter10] = sigmaErr * 17.4533;
	iter10++;
      }
    }
      ...
      for(int k=0; k<entries; k++){
	fitPar->GetEntry(k);
	resolutions_30deg[iter30] = sigma * 17.4533;
	resolutionsErr_30deg[iter30] = sigmaErr * 17.4533;
	iter30++;
      }
    }
      ...
      for(int k=0; k<entries; k++){
	fitPar->GetEntry(k);
	resolutions_50deg[iter50] = sigma * 17.4533;
	resolutionsErr_50deg[iter50] = sigmaErr * 17.4533;
	iter50++;
      }
    }
      ...
      for(int k=0; k<entries; k++){
	fitPar->GetEntry(k);
	resolutions_70deg[iter70] = sigma * 17.4533;
	resolutionsErr_70deg[iter70] = sigmaErr * 17.4533;
	iter70++;
      }
    }
      ...
      for(int k=0; k<entries; k++){
	fitPar->GetEntry(k);
	resolutions_89deg[iter89] = sigma * 17.4533;
	resolutionsErr_89deg[iter89] = sigmaErr * 17.4533;
	iter89++;
      }
    }
  }
  ...
  */
  ...
  gr_10deg->GetYaxis()->SetTitle("#sigma(#Delta#phi) [mrad]");
  gr_10deg->GetYaxis()->SetTitleOffset(1.3);
  //gr_10deg->GetXaxis()->SetRangeUser(0.9,1600);
  gr_10deg->Draw("ap");
  gr_10deg->SetMaximum(5e0);
  gr_10deg->SetMinimum(5e-3);
  ...
  if( inputFile.find(muon) != -1 )
    c->SaveAs("figures/muons/phiRes_vs_pt.eps");
  else if( inputFile.find(electron) != -1 )
    c->SaveAs("figures/electrons/phiRes_vs_pt.eps");
  else if( inputFile.find(pion) != -1 )
    c->SaveAs("figures/pions/phiRes_vs_pt.eps");
