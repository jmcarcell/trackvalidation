...
def main(argv):
        ...
        for var in run_conf["Variables"]:
            if var == "theta":
                for theta in ["10","30","89"] :
                    ...
                    cmd_run_theta = "python submit_allSingleParticles_gps_theta.py -p %s -v %s -r ILCSoft-%s -D %s -j %s -e %s -g %s %s"%(particle, theta, run_conf["Release date"], run_conf["Detector model"],
                                    n_jobs, nev_per_job, eos_output_folder, "" if not eos_custom_lib else (" -l "+eos_custom_lib))
                    print(cmd_run_theta)
                ...
                for energy in ["1","10","100"] :
                    ...
                    cmd_run_energy = "python submit_allSingleParticles_gun_energy.py -p %s -v %s -r ILCSoft-%s -D %s -j %s -e %s -g %s %s"%(particle, energy, run_conf["Release date"], run_conf["Detector model"],
                                     n_jobs, nev_per_job, eos_output_folder, "" if not eos_custom_lib else (" -l "+eos_custom_lib))
                    print(cmd_run_energy)
                    os.system(cmd_run_energy)
            elif var == "pT" or var == "pt":
                step = input("> Which step would you like to run to produce %s at fixed pt ? [sim/ddsim/reco] "%particle)
                for pt in ["1","10","100"] :
                    if str(step) == 'sim':
                        ...
                        cmd_run_pt = "python submit_allSingleParticles_sim_pt.py -p %s -v %s -r ILCSoft-%s -D %s -j %s -e %s -g %s"%(particle, pt, run_conf["Release date"], run_conf["Detector model"],
                        ...
                        cmd_run_pt = "python submit_allSingleParticles_ddsim_pt.py -p %s -v %s -r ILCSoft-%s -D %s -j %s -e %s -g %s -f %s"%(particle, pt, run_conf["Release date"], run_conf["Detector model"],
                        ...
                        #In this step n_jobs is the number of input SIM files taken from "Sample fixed pt SIM folder"
                        #1000 events are always run (10'000 / 1'000 = 10 jobs per file)!
                        cmd_run_pt = "python submit_allSingleParticles_reco_pt.py -p %s -v %s -r ILCSoft-%s -D %s -j %s -e %s -g %s -f %s %s"%(particle, pt, run_conf["Release date"], run_conf["Detector model"],
                                         n_jobs, "1000", eos_output_folder, run_conf["Sample fixed pt SIM folder"], "" if not eos_custom_lib else (" -l "+eos_custom_lib))
                        print(cmd_run_pt)
                        os.system(cmd_run_pt)
                    else:
    ...
    for particle in run_conf["Particles"]:
        ...
        if run_conf["Type sample ttbar"] == "SIM":
            ...
            cmd_run_ttbar = "python submit_ttbar_fromSIM.py -p %s -f %s -r ILCSoft-%s -D %s -j %s -e %s -g %s %s"%(particle, run_conf["Input sample ttbar"], run_conf["Release date"], run_conf["Detector model"],
                            n_jobs, nev_per_job, eos_output_folder, "" if not eos_custom_lib else (" -l "+eos_custom_lib))
            print(cmd_run_ttbar)
        ...
        if run_conf["Type sample ttbar"] == "REC":
            ...
            cmd_run_ttbar = "python submit_ttbar_fromREC.py -p %s -f %s -r ILCSoft-%s -D %s -j %s -e %s -g %s %s"%(particle, run_conf["Input sample ttbar"], run_conf["Release date"], run_conf["Detector model"],
                            n_jobs, nev_per_job, eos_output_folder, "" if not eos_custom_lib else (" -l "+eos_custom_lib))
            print(cmd_run_ttbar)
            os.system(cmd_run_ttbar)
...
if __name__ == "__main__":
    ...
    check_dirac = input("Did you setup Dirac? [yes/no] ")
